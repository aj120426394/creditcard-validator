...
module.exports = class Util {
  ...
  static extractLibBundle(bundles) {
    return ({
  ...
  static SWPrecacheWebpackPlugin(publicPath) {
    return {
      plugins: [
        new SWPrecacheWebpackPlugin({
          cacheId: 'the-griffith-graduate',
          dontCacheBustUrlsMatching: /\.\w{8}\./,
          filename: 'service-worker.js',
          minify: true,
          navigateFallback: publicPath + 'index.html',
          staticFileGlobsIgnorePatterns: [/\.map$/, /.*manifest\.json$/, /\.htaccess/]
        }),
        new ManifestPlugin({
  ...
  static AssetsManifest(seed = {}) {
    return {
      plugins: [
        new WebpackPwaManifest(seed)
      ]
    };
  }
};
